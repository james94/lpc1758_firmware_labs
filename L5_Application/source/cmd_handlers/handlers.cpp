...
CMD_HANDLER_FUNC(test)
{
	puts("test testing okay");
	return true;
}

CMD_HANDLER_FUNC(controlTaskHandler)
{

	output.putline("Entered Control Task Handler");
    // point to task name string represents consumer or producer task
    scheduler_task *pTaskProducer = scheduler_task::getTaskPtrByName("producer");

    // point to task name string represents consumer or producer task
    scheduler_task *pTaskConsumer = scheduler_task::getTaskPtrByName("consumer");

    //resume either producer or consumer, depends which name is given
    if (cmdParams == "resume producer") {
        //if task_name is producer resumes it, else consumer resumed
        pTaskProducer->resume();
    }
    else if (cmdParams == "suspend producer") {
    	//if task_anem is producer suspends it, else consumer suspended
    	pTaskProducer->suspend();
    }
    else if(cmdParams == "resume consumer") {
    	//if task_anem is producer suspends it, else consumer suspended
    	pTaskConsumer->resume();
    }
    else if(cmdParams == "suspend consumer") {
    	//if task_anem is producer suspends it, else consumer suspended
    	pTaskConsumer->suspend();
    }
