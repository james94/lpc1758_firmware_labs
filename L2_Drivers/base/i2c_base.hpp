...
class I2C_Base
{
    public:
        ...
        /**
         * Initializes I2C Slave Device on Communication BUS
         * @param slaveAddr the slave address
         * @param pBufferData a pointer to the read/write buffer of the slave device
         * @param bufferSize defines the size of the slave buffer
         */
        bool initSlave(uint8_t slaveAddr, uint8_t *pBufferData, uint32_t bufferSize);

        /**
         * This structure contains I2C Slave Device transaction parameters
         */
        typedef struct
        {
            uint32_t slaveTrxSize;   ///< # of bytes to transfer.
            uint8_t slaveAddr;  ///< Slave Device Address
            uint8_t slaveFirstReg;   ///< 1st Register to Read or Write
            uint8_t slaveError;      ///< Error if any occurred within I2C
            uint8_t *pSlaveData;  ///< Buffer of the I2C Read or Write
        } mI2CSlaveTransaction_t;

        /// The I2C Input Output frame that contains I2C slave transaction information
        mI2CSlaveTransaction_t mSlaveTransaction;

        // Slave pRegMap
        uint8_t *pRegMap;
        // Slave data counter
        uint32_t mSlaveCounter;
